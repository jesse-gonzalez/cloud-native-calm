...
cat ~/.ssh/${INSTANCE_NAME}_wildcard_karbon_ca.crt | openssl x509 -text -noout | grep DNS

echo ""
echo "Register JFrog Container Registry to Karbon - noip.io scenario"
karbonctl registry add --name ${INSTANCE_NAME}_noip --url ${INSTANCE_NAME}.${NIPIO_INGRESS_DOMAIN} --cert-file ~/.ssh/${INSTANCE_NAME}_noip_karbon_ca.crt --username admin --password @@{Artifactory Credential.secret}@@
monitor_registry_add_task

echo ""
echo "Register JFrog Container Registry to Karbon - wildcard scenario"
karbonctl registry add --name ${INSTANCE_NAME}_wildcard --url ${INSTANCE_NAME}.${WILDCARD_INGRESS_DNS_FQDN} --cert-file ~/.ssh/${INSTANCE_NAME}_wildcard_karbon_ca.crt --username admin --password @@{Artifactory Credential.secret}@@
monitor_registry_add_task
