...
kubectl create secret docker-registry ${INSTANCE_NAME}-wildcard-docker-registry-cred --docker-server=${INSTANCE_NAME}.${WILDCARD_INGRESS_DOMAIN} --docker-username=admin --docker-password='@@{Artifactory Credential.secret}@@' -n ${NAMESPACE}
