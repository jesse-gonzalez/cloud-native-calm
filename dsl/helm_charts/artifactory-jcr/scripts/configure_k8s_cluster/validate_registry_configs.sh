...
kubectl get secrets ${INSTANCE_NAME}-noip-tls -o jsonpath='{.data.ca\.crt}' | base64 -d > ~/.ssh/karbon_ca.crt
kubectl get secrets ${INSTANCE_NAME}-noip-tls -o jsonpath='{.data.tls\.key}' | base64 -d > ~/.ssh/karbon_artifactory_tls.key
kubectl get secrets ${INSTANCE_NAME}-noip-tls -o jsonpath='{.data.tls\.crt}' | base64 -d > ~/.ssh/karbon_artifactory_tls.crt

# mv certs to /etc/docker/certs.d directory
sudo mkdir -p /etc/docker/certs.d/${INSTANCE_NAME}.${NIPIO_INGRESS_DOMAIN}/;sudo mv ~/.ssh/karbon_artifactory_tls.crt /etc/docker/certs.d/${INSTANCE_NAME}.${NIPIO_INGRESS_DOMAIN}/ca.crt;
...
kubectl create secret docker-registry jcr-docker-registry -n default --docker-server=${INSTANCE_NAME}.${NIPIO_INGRESS_DOMAIN} --docker-username=admin --docker-password='@@{Artifactory Credential.secret}@@' --docker-email=no-reply@nutanix.com;
...
helm repo add --ca-file ~/.ssh/${INSTANCE_NAME}_karbon_ca.crt jcr-helm https://${INSTANCE_NAME}.${NIPIO_INGRESS_DOMAIN}/artifactory/helm --username admin --password '@@{Artifactory Credential.secret}@@'
helm repo update
