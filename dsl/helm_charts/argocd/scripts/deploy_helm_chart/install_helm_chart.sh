...
helm upgrade --install ${INSTANCE_NAME} argo/argo-cd \
	...
	--set server.ingress.hosts[1]="${INSTANCE_NAME}.${WILDCARD_INGRESS_DOMAIN}" \
	--set server.ingress.tls[1].hosts[0]=${INSTANCE_NAME}.${WILDCARD_INGRESS_DOMAIN} \
...
Alternatively, if DNS wildcard domain configured, navigate to https://${INSTANCE_NAME}.${WILDCARD_INGRESS_DOMAIN}
