...
kubectl create clusterrolebinding ${USER_NAME}-rb --clusterrole=cluster-admin --serviceaccount=${NAMESPACE}:${USER_NAME} --namespace=${NAMESPACE}

##  Get the service account secret name
SA_SECRET_NAME=$(kubectl get serviceaccounts ${USER_NAME} -o jsonpath='{.secrets[].name}' -n ${NAMESPACE});
...
kubectl config set-context ${USER_NAME}@${K8S_CLUSTER_NAME} --cluster=${K8S_CLUSTER_NAME} --user=${USER_NAME} --namespace=${NAMESPACE}

##  Use context and set namespace to correct namespace
kubectl config use-context ${USER_NAME}@${K8S_CLUSTER_NAME}
